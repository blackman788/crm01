     ...
     $.ajax({
         ...
         'success':function(result){
             if(200 == result.code){
                ...
                console.log(object.chance_source)
             }
...
function delete_sales(){
    var idx = $('#dg').datagrid('getChecked')
    /* var data=new Array()         //该方法不可行 传过去['':'','':''] 的字符串难以切割
    for(i=0;i<idx.length;i++){
        data.push(idx[i].id)
    }
    data01 = JSON.stringify(data) //字典转字符串 */
    $.messager.confirm('Confirm', '你确定要删除这'+ idx.length +'条数据吗', function(r){
        if (r){
            var sql = ''
            for(i=0;i<idx.length;i++){
                if(i == idx.length-1){
