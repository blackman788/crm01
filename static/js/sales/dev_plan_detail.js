...
$('#tg').edatagrid({
	...
	updateUrl: '/sales/update_dev_plan/',
...
function change_devresult(state){
    $.messager.confirm('确认','本次操作不可逆,确定要更改吗？',function(r){
        if (r){
            $.get(
                '/sales/dev_plan_change_state/' +'?state=' + state + '&sales_chance_id='+ $('#sales_chance_id').val(),
                function(result){
