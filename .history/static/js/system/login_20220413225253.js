    ...
    $.ajax({
        ...
        'success':function(result){
            ...
            //$('#login_span').html(result.msg);
                if (!(undefined == result.login_cookies || null == result.login_cookies)) {
                var estr = $.base64.encode(result.login_cookies);
                $.cookie('login_cookie', estr, { expires: 5 ,'path': '/'});
                }
...
$('#login_btn').on('click',check_login_btn)

//进入页面就执行的方法 
//若页面存在cookie直接跳转
$(function(){
    var uu1 = $.cookie('login_cookie')
    if (!(undefined == uu1 || null == uu1)){
        uu2 = $.base64.decode(uu1)
        $('#login_username').val(uu2.split('&')[0])
        $('#login_password').val(uu2.split('&')[1])
