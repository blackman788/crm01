...
def select_dev_plan_detail(request,id):
    ...
    try:
        ...
        #关闭游标
        cursor.close()
        return render(request,'SaleChance/dev_plan_detail.html',{'dt':object_list,'flag':flag})
...
def add_dev_plan(request,id):
    try:
        ...
        #添加成功后还要在营销计划表中将  devResult 更改为  1  开发中
        SaleChance.objects.filter(pk=id).update(devResult=1)
...
#开发成功和开发失败. 开发成功sale_chance 表内的devresult字段更改为2 失败为3
@require_GET
def dev_plan_change_state(request):
    state = request.GET.get('state')
    sales_chance_id = request.GET.get('sales_chance_id')
    SaleChance.objects.filter(pk=sales_chance_id).update(devResult=state)
    return JsonResponse({'code':'200','msg':'添加成功'})