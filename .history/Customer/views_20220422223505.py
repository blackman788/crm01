...
def inquire_reprieve(request):
    id = request.GET.get('id')
    c1= CustomerLoss.objects.get(pk=id)
    c = CustomerReprieve.objects.values().filter(customerLoss=id).order('-id')
    return render(request,'Customer/order.html',{'code':'200','msg':'查询成功','cl':c})
