...
from Customer.models import Customer,LinkMan,Contact,CustomerOrders,OrdersDetail,CustomerLoss
...
@csrf_exempt
@require_POST
def loss_detail(request,id):
    #获取第几页
        page =request.POST.get('page')
        page_size=request.POST.get('rows')

        #返回多条结果行
        object_list = CustomerLoss.objects.values().filter(customer=c).order_by('-id')
